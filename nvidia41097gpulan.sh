...
# sudo bash nvidia41097gpulan.sh
# OR
# su
# bash nvidia41097gpulan.sh
# OR
# sudo chmod 755 nvidia41097gpulan.sh
# sudo ./nvidia41097gpulan.sh
# OR
# su
# chmod 755 nvidia41097gpulan.sh
# ./nvidia41097gpulan.sh
...
if [ "$BASH_VERSION" = '' ]; then
  ...
  echo "sudo bash nvidia41097gpulan.sh"
  exit 1
fi
...
if [[ $(which dkms | wc -l) -gt 0 ]]; then
  sh NVIDIA-Linux-x86_64-410.97.run --accept-license -q --dkms -X -Z --ui=none -s
else
  sh NVIDIA-Linux-x86_64-410.97.run --accept-license -q -X -Z --ui=none -s
  # sh NVIDIA-Linux-x86_64-410.97.run --accept-license -q -X -Z
fi
echo \ 
